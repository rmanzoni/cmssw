...
class recoilComponent {
 ...
 public:
  ...
  int charge() const { return this->srcLepton_->charge(); }
  bool isMuon() const { return (abs(pdgId()) == 13); }
  bool isElectron() const { return (abs(pdgId()) == 11); }
};
...
class recoilingBoson : public reco::Particle {
 ...
 public: 
  ...
  bool isDiElectron() const { return (leptons.size() == 2) ? (leptons[0].isElectron() and leptons[1].isElectron()) : false; }
  int select() const { return (this->p4vec().M() > 80 && this->p4vec().M() < 100 && this->isDiMuon()) + (this->p4vec().M() > 80 && this->p4vec().M() < 100 && this->isDiElectron())*2; }
  ...
  //clean if dR < 0.4 between lepton and jet candidate
  bool isJetNotALepton(reco::Candidate::LorentzVector jetCandidate) const
  {
      bool jetIsNotLepton = true;
      for(const auto & lepton: leptons)
	  if (deltaR2 (lepton.p4(),jetCandidate) < 0.16)
	      jetIsNotLepton = false;
      
      return jetIsNotLepton;
  ...
  reco::Candidate::LorentzVector getDaughterP4(int index) const  { return leptons[index].p4(); }
  int getDaughterCharge(int index) const { return leptons[index].charge(); }
...
//   void doCombinations(int offset, int k);
  ...
  void handleElectrons(edm::Ptr<reco::Candidate> lepton, recoilingBoson& Z, const pat::ElectronCollection& );
  void handleTaus(edm::Ptr<reco::Candidate> lepton, recoilingBoson& Z, const pat::TauCollection& );
  void fillEventInformation(edm::Event&, const recoilingBoson &Z);
